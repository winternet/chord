...
class grpc_exception : public exception {
 private:
  grpc::Status status;
 public:
  explicit grpc_exception(std::string message, grpc::Status status)
      : exception(message), status(std::move(status)) {}

  explicit grpc_exception(std::string message, grpc::Status status, std::string file, size_t line)
      : exception(message, file, line), status(std::move(status)) {}

...
using chord::test::p;
using chord::test::fn;
using chord::test::mkdir;

class MkDirTest : public chord::test::IntegrationTest {
  void SetUp() override {
...
TEST_F(MkDirTest, nodes_1__repl_1__folder_root) {
  ...
  const auto empty_folder_uri = uri{"chord:///folder"};

  ASSERT_TRUE(file::is_empty(data0->path));

  const auto status = mkdir(peer, 1, empty_folder_uri);
  ASSERT_TRUE(status.ok());

  // folder has only been added to the metadata
  ASSERT_TRUE(peer->get_metadata_manager()->exists(empty_folder_uri));
  const auto root_meta = peer->get_metadata_manager()->get(root_uri);

  assert_metadata(peer, empty_folder_uri);
  //const auto empty_folder_meta = std::find_if(root_meta.begin(), root_meta.end(),[&](const auto& m) {return m.name == empty_folder_uri.path().filename(); });
  //ASSERT_TRUE(empty_folder_meta != root_meta.end());

  // folder is not actually created
  ASSERT_TRUE(file::is_empty(data0->path));
}

TEST_F(MkDirTest, nodes_1__repl_1__folder_root_already_exists) {
  const auto data0 = base.add_dir("data0");
  const auto meta0 = base.add_dir("meta0");
  const auto port = 50050;
  const auto peer = make_peer(test::make_context({"0"}, {bind_addr+std::to_string(port)}, data0, meta0));
  ...
  const std::set<Metadata> metadata{MetadataBuilder::directory(empty_folder_uri)};
  ASSERT_TRUE(peer->get_metadata_manager()->add(empty_folder_uri, metadata));
  ASSERT_TRUE(file::is_empty(data0->path));

  const auto status = mkdir(peer, 1, empty_folder_uri);

  ASSERT_FALSE(status.ok());
  ASSERT_EQ(status.error_code(), grpc::StatusCode::ALREADY_EXISTS);

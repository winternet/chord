...
using chord::test::p;
using chord::test::fn;
using chord::test::mov;
using chord::utils::as_uri;

class MovTest : public chord::test::IntegrationTest {
  void SetUp() override {
    logger = chord::log::get_or_create("int.test.mov");
  }
...
TEST_F(MovTest, nodes_1__file_not_found) {
  ...
  const auto root_uri = as_uri("/");

  ASSERT_TRUE(file::is_empty(data0->path));
  const auto status = mov(peer, as_uri("/file0"), as_uri("/file1"));
  ASSERT_FALSE(status.ok());
}

TEST_F(MovTest, nodes_1__repl_1__file_1) {
  const auto source0 = base.add_dir("source0");
  const auto file0 = source0->add_file("file0");
  ...
  assert_equal(peer, file0, uri{"chord:///file1"}, true);
}

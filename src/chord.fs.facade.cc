...
void Facade::dir(const chord::uri &uri, iostream &iostream) {
  ...
  if(!status.ok()) throw chord::exception("failed to dir " + to_string(uri), status);
}

void Facade::del(const chord::uri &uri) {
  auto status = fs_client->del(uri);
  if(!status.ok()) throw chord::exception("failed to del file " + to_string(uri), status);

  //status = fs_client->meta(uri, Client::Action::DEL);
  //if(!status.ok()) throw chord::exception("failed to del metadata " + to_string(uri), status);
}

...
Peer::Peer(shared_ptr<Context> context)
    : scheduler{new Scheduler()}, context{context}, router{make_unique<Router>(context.get())},
      client{make_unique<Client>(*context, *router)}, service{make_unique<Service>(*context, *router)},
